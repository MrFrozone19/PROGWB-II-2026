/* Pantalla 02 — Registro de empresa. */
...
function actualizarCobertura() {
  ...
  if (municipio) {
    ...
    mensajeCobertura.textContent =
      'Este código postal está fuera del área metropolitana. Solo pueden registrarse empresas dentro de la zona.';
  }
}

/* Devuelve una lista de [idDelCampo, mensaje] con cada error encontrado. */
function validarRegistro() {
  ...
  if (!esRfcValido(valorDe('rfc'))) {
    errores.push(['rfc', 'El RFC debe tener 12 o 13 caracteres, por ejemplo ADN180423K21.']);
  }
  ...
  if (!buscarMunicipioPorCodigoPostal(codigoPostal)) {
    // Si ya tiene 5 dígitos, el aviso de cobertura explica el problema.
    const mensaje = codigoPostal.length === 5 ? '' : 'Escribe un código postal de 5 dígitos.';
    errores.push(['codigo-postal', mensaje]);
  }

  exigir('nombre-responsable', 'Escribe el nombre de la persona responsable.');
  if (!esTelefonoValido(valorDe('telefono'))) {
    errores.push(['telefono', 'Escribe un teléfono de 10 dígitos.']);
  }
  if (!esCorreoValido(valorDe('correo'))) {
    errores.push(['correo', 'Escribe un correo válido, por ejemplo compras@empresa.mx.']);
  }
...
formularioRegistro.addEventListener('submit', (evento) => {
  ...
  // Prototipo: el registro no se guarda; solo se muestra la confirmación.
